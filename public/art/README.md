...
| `ui.plate.<gold,cyan,pink,plain,gold.pressed>` | 9-slice button plates (`ui/plate_*.webp`, keyed, ~1000×305). The loader publishes each as a CSS variable (`cssVar`) and adds `body.uiArt` once the four main plates are in; `.btn` then uses `border-image` with slices 72/170/75/140 and a `round` middle. Ornament must stay in the corners and edges with a plain centre. |
| `ui.panel` | 9-slice iron frame (`ui/panel_frame.webp`, slice 130) around `.box` and `.card` panels, centre not filled. |
| `ui.divider`, `ui.icons.sections` | Guide ornaments: the chain rule under each heading and the 7 section icons (book, skull, weapons, map, hooded, gear, hourglass) mapped to headings by order. |
| `ui.guide.landscape` | How-to-play backdrop on wide screens (cover); portrait relies on the panel frame alone. |
